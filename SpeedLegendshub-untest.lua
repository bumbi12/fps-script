--[[
  ...
  Phiên bản: 5.0 Ultimate
  Tính năng:
    - Giao diện cao cấp với animation mượt
    - Màn hình phủ khi bật chức năng
    - Auto farm ngẫu nhiên toàn bộ map
...
local function CollectItem(item)
    if item and item:IsA("BasePart") and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        CounterText.Text = "ĐÃ NHẶT: "..CollectedItems
        return true
    end
    return false
end
...
local function AutoFarm()
    while Running and task.wait() do
        ...
        if targetItem then
            ...
            StatusLabel.Text = "🔴 LỖI: KHÔNG TÌM THẤY VẬT PHẨM"
            StatusLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
            task.wait(1)
        end
    end
...
StartButton.MouseButton1Click:Connect(function()
    ...
    StatusLabel.Text = "🟠 ĐANG CHẠY AUTO FARM"
...
task.delay(1.5, function()
    StatusLabel.Text = "🟢 SẴN SÀNG HOẠT ĐỘNG"
end)
